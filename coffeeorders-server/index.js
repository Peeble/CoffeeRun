...
app.get('/coffeeorders/:emailAddress', (req, res) => {
  ...
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.find({ emailAddress: emailAddress })
...
app.put('/coffeeorders/:emailAddress', (req, res) => {
  ...
  console.log(`looking for: ${emailAddress}`)
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.findOne({ emailAddress: emailAddress })
...
app.get('/coffeeorders/coffee/:coffee', (req, res) => {
  ...
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.find({ coffee: coffee })
  if (record) res.status(200).json(record)
  else res.sendStatus(404)
})

app.put('/coffeeorders/coffee/:coffee', (req, res) => {
  const coffee = req.params.coffee
  console.log(`looking for: ${coffee}`)
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.findOne({ coffee: coffee })
  console.log(`PUT: ${JSON.stringify(record, null, 2)}`)
  if (record) {
    try {
      req.body._id = record._id
...
app.get('/coffeeorders/flavor/:flavor', (req, res) => {
  ...
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.find({ flavor: flavor })
  if (record) res.status(200).json(record)
  else res.sendStatus(404)
})

app.put('/coffeeorders/flavor/:flavor', (req, res) => {
  const flavor = req.params.flavor
  console.log(`looking for: ${flavor}`)
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.findOne({ flavor: flavor })
  console.log(`PUT: ${JSON.stringify(record, null, 2)}`)
  if (record) {
    try {
      req.body._id = record._id
...
app.get('/coffeeorders/strength/:strength', (req, res) => {
  ...
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.find({ strength: strength })
  if (record) res.status(200).json(record)
  else res.sendStatus(404)
})

app.put('/coffeeorders/strength/:strength', (req, res) => {
  const strength = req.params.strength
  console.log(`looking for: ${strength}`)
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.findOne({ strength: strength })
  console.log(`PUT: ${JSON.stringify(record, null, 2)}`)
  if (record) {
    try {
      req.body._id = record._id
...
app.get('/coffeeorders/size/:size', (req, res) => {
  ...
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.find({ size: size })
  if (record) res.status(200).json(record)
  else res.sendStatus(404)
})

app.put('/coffeeorders/size/:size', (req, res) => {
  const size = req.params.size
  console.log(`looking for: ${size}`)
  db.connect('./data', ['coffeeorders'])
  const record = db.coffeeorders.findOne({ size: size })
  console.log(`PUT: ${JSON.stringify(record, null, 2)}`)
  if (record) {
    try {
      req.body._id = record._id
